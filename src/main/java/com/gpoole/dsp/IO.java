...
public class IO extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        startBut.setText("Capture Audio");
    ...
    private void startButActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_startButActionPerformed
        try {
            ...
            //int sampleRate = Integer.parseInt(this.freqComboBox.getSelectedItem().toString());
            //int bytes = Integer.parseInt((String) this.bytesjSpinner1.getValue());
            AudioFormat format = new AudioFormat((float) 48000, 16, channels, true, true);
            TargetDataLine microphone = AudioSystem.getTargetDataLine(format);
            AudioInputStream ais = new AudioInputStream(microphone);
            microphone.open();
            microphone.start();
            
            this.SampleTextField.setText(String.valueOf((int) (1000000 * (float) 1 / Integer.parseInt(freqComboBox.getSelectedItem().toString()))) + "us");
            this.BitsTextField.setText(String.valueOf(bits));
            this.ChannelsTextField.setText(String.valueOf(channels));
            by = null;
            by = new byte[4 * Integer.parseInt(this.DFTSizeComboBox.getSelectedItem().toString())];
            Thread t1 = new Thread(() -> {

                FrequencyScanner fs = new FrequencyScanner();
                while (open) {
                    if (microphone.available() >= Integer.parseInt(IO.this.DFTSizeComboBox.getSelectedItem().toString())) {
                        try {
                            by = null;
                            by = new byte[4 * Integer.parseInt(IO.this.DFTSizeComboBox.getSelectedItem().toString())];
                            double[] shorts = new double[by.length / 4];
                            //if(microphone.available()-by.length < 0){microphone.read(by, microphone.available()-by.length, by.length);}
                            //else{microphone.read(by, 0, by.length);}
                            microphone.read(by, 0, by.length);
                            //microphone.flush();
                            for (int index = 0; index < by.length; index++) {
                                if (!((index << 2 + 1) > by.length)) {
                                    for (int d = 0; d < by.length<<2; d++) {
                                        shorts[index] = (short) (by[index << 1] << 8);
                                        shorts[index] = (short) (by[index << 1 + 1] + shorts[index]);
                                    }
                                }
                                IO.this.freqTextField.setText(String.valueOf((double) fs.extractFrequency(shorts, 8000)) + "Hz");
                            }
    ...
    public class FrequencyScanner {
        ...
        public double extractFrequency(double[] sampleData, int sampleRate) throws InterruptedException, IOException {
            ...
            IO.this.PeriodTextField.setText(String.valueOf(endTime - startTime) + "ms");
        ...
        private List<CLDevice> getDevices() {
            ...
            for (CLPlatform platform : JavaCL.listPlatforms()) {
                for (CLDevice device : platform.listAllDevices(true)) {
                    devices.add(device);
                    IO.this.VendorTextField.setText(device.getVendor());
                    IO.this.processorTextField1.setText(device.getName());
                    IO.this.OpenCLTextField.setText(device.getOpenCLCVersion());
                    IO.this.DriverTextField1.setText(device.getDriverVersion());
                }
            }
            return devices;
        }
